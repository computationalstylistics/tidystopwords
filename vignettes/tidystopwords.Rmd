---
title: "Multilingual Stopwords with the Package 'tidystopwords'"
...
This vignette explains basic functionalities of the package `tidystopwords`. The idea behind this package is to give the user control over the stop-word selection, which is one of the most important steps in any text-mining analysis.
...
of more fine-grained morphological categories are stored in the __Universal Features__. 
...
package or viewing this vignette on a Windows-operated PC with a Latin-alphabet locale. 
With a Latin-alphabet locale, we have observed that a non-Latin vector output displays 
correctly both in RMarkdown and in the console, while the same output displays as 
a set of Unicode codes when in the form of data frame. 
